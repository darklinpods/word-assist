...
首次打开插件时，如果没有检测到 AI 配置，会自动出现新手引导。按照页面提示：

1. 登录火山方舟控制台并开通服务。
2. 在“API Key 管理”中创建并复制 API Key。
3. 在“在线推理”中选择文本对话模型、创建并启动推理接入点，复制以 `ep-` 开头的 ID。
4. 回到插件填入两项信息，点击“验证并保存配置”。

配置验证成功后保存在当前电脑的插件本地存储中，不会写入 Word 文档或项目代码。API Key 属于敏感凭证，不要在公用电脑保存，也不要转发给其他人。之后可通过插件右上角的“AI 配置”修改。
...
> **提示**：`.env.local` 只在 Vite 本地开发模式下生效，生产构建不会携带开发者的密钥。运行时保存的个人配置优先；可在插件右上角进入“AI 配置”进行覆盖和连通性验证。
