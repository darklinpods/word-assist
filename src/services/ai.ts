...
【防幻觉硬约束（必须遵守）】
1. 只提取原文明确出现的信息，不得编造或推断原文没有的事实、票据金额、日期、保单号、姓名、车号、保险险种与保额等任何信息。
2. 不得改动原文认定的核心事实（时间、地点、当事人、车辆、责任划分）。
3. 不得补充原文未引用的法条具体条款号。
4. 缺失的字段一律输出空字符串；原文未出现的角色输出空数组。禁止输出【】占位符、注释或任何解释性文字。
5. 不得依据任何外部数据补全信息，只以用户提供的诉状文本为准。
6. 多个当事人必须分别输出。
诉讼请求、事故事实、责任认定、投保情况、其他事实和索赔清单应尽量保持原文措辞，不得润色或改写。
