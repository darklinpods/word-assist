import { FileDown, Loader2, CheckCircle2, Calculator } from 'lucide-react';
  ...
  return (
    <div className="mt-4 border-t border-gray-100 pt-4 space-y-3 animate-in fade-in slide-in-from-bottom-2 duration-300">
      ...
      <div className="flex items-center justify-between">
        <h3 className="text-[14px] font-bold text-text-primary font-heading flex items-center gap-1.5">
          <Calculator className="w-4 h-4 text-primary" />计算结果
        </h3>
      ...
      <div className="space-y-0 border border-gray-100 rounded-lg overflow-hidden text-[12px]">
        ...
        {enabledItems.map(item => (
          ...
          >
            <CheckCircle2 className="w-3 h-3 text-emerald-500 flex-shrink-0 mt-0.5 mr-2" />
            <div className="flex-1 min-w-0">
              <div className="font-semibold text-text-primary">{item.type}</div>
              {item.formula !== '—' && (
                <div className="text-[10px] text-text-muted mt-0.5 break-words leading-tight">
                  {item.formula}
                </div>
              )}
            </div>
            <span className="font-mono font-bold text-text-primary ml-2 whitespace-nowrap">
        ...
        {items.filter(i => !i.enabled).length > 0 && (
          <details className="group">
            <summary className="px-3 py-1.5 text-[10px] text-text-muted cursor-pointer hover:bg-gray-50 list-none flex items-center">
      ...
      <button
        ...
        className="w-full py-2.5 bg-primary hover:bg-primary-dark disabled:bg-gray-300 text-white text-sm font-medium rounded-lg flex items-center justify-center transition-colors shadow-sm cursor-pointer"
      >
        {isExporting ? (
